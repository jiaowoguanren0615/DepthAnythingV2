...
from util.losses import gradient_matching_loss, scale_and_shift_invariant_loss




#  TODO https://arxiv.org/abs/2406.09414  section 7.1
def criterion(inputs, target, loss_weight=None, ignore_index: int = -100):
    ...
    # gm_loss = gradient_matching_loss(inputs, target)
    # ssi_loss = scale_and_shift_invariant_loss(inputs, target)

    losses['ce_loss'] = ce_loss

    # The weight ratio of Lssi and Lgm is set as 1:2
    # losses['gm_ssi'] = 1.0 * ssi_loss + 2.0 * gm_loss
    # if including_ce:
    #     return losses['gm_ssi'] + losses['ce_loss']
    return losses['ce_loss']
...
def train_one_epoch(model, optimizer, dataloader,
    ...
    for idx, (img, lbl) in enumerate(metric_logger.log_every(dataloader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            logits = model(img)
            loss = criterion(logits, lbl, loss_weight, ignore_index=args.ignore_index)
